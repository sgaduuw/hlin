...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Backslash variants: Werkzeug emits them verbatim and browsers
        # normalise "\" to "/", so these would otherwise reach //evil.com.
        ("/\\evil.com", None),
        ("\\\\evil.com", None),
        ("/\\/evil.com", None),
        # Control characters and surrounding whitespace must not slip through.
        ("/foo\r\nSet-Cookie: x", None),
        (" /foo", None),
        ("/foo\t", None),
        ("", None),
...
def test_login_rejects_unknown_username(client):
    # No user created: the missing-username path must also 401 (and runs the
    # dummy-hash compare so it is not distinguishable by timing).
    assert _login(client, username="ghost").status_code == 401


def test_verify_password_handles_missing_user():
    assert auth.verify_password(None, "anything") is False


# --- secret-key resolution (stable across gunicorn workers) -------------


def test_resolve_secret_key_prefers_configured(monkeypatch):
    import hlin
    from hlin.settings import settings

    monkeypatch.setattr(settings, "secret_key", "configured-key")
    assert hlin._resolve_secret_key() == "configured-key"


def test_resolve_secret_key_persists_when_unset(tmp_path, monkeypatch):
    import hlin
    from hlin.settings import settings

    monkeypatch.setattr(settings, "secret_key", None)
    monkeypatch.setattr(settings, "db_path", str(tmp_path / "hlin.db"))
    first = hlin._resolve_secret_key()
    second = hlin._resolve_secret_key()
    # A second worker building the app must converge on the same key, or a
    # cookie minted by one worker fails on another.
    assert first == second
    assert (tmp_path / ".hlin-secret-key").exists()
